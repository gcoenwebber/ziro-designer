...
import type { LibGraphic, LibPin, LibSymbol, LibSymbolUnit, SchField, Stroke, Fill, TextEffects, Vec2 } from './types.js';
...
function strokeNode(stroke: Stroke | undefined): SList {
  ...
  if (stroke?.color) {
    ...
    items.push(list(atom('color'), atom(String(r)), atom(String(g)), atom(String(b)), atom(String(a))));
  }
...
function effectsNode(effects: TextEffects | undefined): SList {
  ...
  if (justify.length > 0) items.push({ kind: 'list', items: [atom('justify'), ...justify.map((t) => atom(t))] });
...
export function buildLibPinNode(pin: Omit<LibPin, 'source'>, prevSource?: SList): SList {
  const items: SNode[] = [
    atom('pin'), atom(pin.electricalType), atom(pin.shape),
  ...
  items.push(list(atom('name'), str(pin.name),
    list(atom('effects'), list(atom('font'), list(atom('size'), atom(mm(nameSize)), atom(mm(nameSize)))))));
  items.push(list(atom('number'), str(pin.number),
    list(atom('effects'), list(atom('font'), list(atom('size'), atom(mm(numSize)), atom(mm(numSize)))))));
  if (prevSource) {
...
export function buildLibGraphicNode(g: LibGraphic): SList {
  switch (g.kind) {
    case 'rectangle':
      return list(atom('rectangle'),
        list(atom('start'), atom(fx(g.start)), atom(fy(g.start))),
        list(atom('end'), atom(fx(g.end)), atom(fy(g.end))),
        strokeNode(g.stroke), fillNode(g.fill));
    case 'circle':
      return list(atom('circle'),
        list(atom('center'), atom(fx(g.center)), atom(fy(g.center))),
        list(atom('radius'), atom(mm(g.radius))),
        strokeNode(g.stroke), fillNode(g.fill));
    case 'arc':
      return list(atom('arc'),
        list(atom('start'), atom(fx(g.start)), atom(fy(g.start))),
        list(atom('mid'), atom(fx(g.mid)), atom(fy(g.mid))),
        list(atom('end'), atom(fx(g.end)), atom(fy(g.end))),
        strokeNode(g.stroke), fillNode(g.fill));
    case 'polyline':
      return list(atom('polyline'),
        { kind: 'list', items: [atom('pts'), ...g.points.map((p) => list(atom('xy'), atom(fx(p)), atom(fy(p))))] },
        strokeNode(g.stroke), fillNode(g.fill));
    case 'text':
      return list(atom('text'), str(g.text),
...
const vecEq = (a: Vec2 | undefined, b: Vec2 | undefined): boolean =>
  a?.x === b?.x && a?.y === b?.y;

const strokeEq = (a: Stroke | undefined, b: Stroke | undefined): boolean =>
  (a?.width ?? 0) === (b?.width ?? 0) && (a?.type ?? 'default') === (b?.type ?? 'default')
  && JSON.stringify(a?.color) === JSON.stringify(b?.color);

const fillEq = (a: Fill | undefined, b: Fill | undefined): boolean =>
  (a?.type ?? 'none') === (b?.type ?? 'none') && JSON.stringify(a?.color) === JSON.stringify(b?.color);
...
function pinNode(pin: LibPin): SList {
  ...
  const same = orig.electricalType === pin.electricalType && orig.shape === pin.shape
    && vecEq(orig.at, pin.at) && orig.angle === pin.angle && orig.length === pin.length
    && orig.name === pin.name && orig.number === pin.number && orig.hidden === pin.hidden
    && orig.nameSize === pin.nameSize && orig.numberSize === pin.numberSize;
...
function graphicNode(g: LibGraphic): SList {
  ...
  if (orig !== undefined && orig.kind === g.kind) {
    ...
    if (orig.kind === 'rectangle' && g.kind === 'rectangle')
      same = vecEq(orig.start, g.start) && vecEq(orig.end, g.end) && strokeEq(orig.stroke, g.stroke) && fillEq(orig.fill, g.fill);
    else if (orig.kind === 'circle' && g.kind === 'circle')
      same = vecEq(orig.center, g.center) && orig.radius === g.radius && strokeEq(orig.stroke, g.stroke) && fillEq(orig.fill, g.fill);
    else if (orig.kind === 'arc' && g.kind === 'arc')
      same = vecEq(orig.start, g.start) && vecEq(orig.mid, g.mid) && vecEq(orig.end, g.end) && strokeEq(orig.stroke, g.stroke) && fillEq(orig.fill, g.fill);
    else if (orig.kind === 'polyline' && g.kind === 'polyline')
      same = orig.points.length === g.points.length && orig.points.every((p, i) => vecEq(p, g.points[i]))
        && strokeEq(orig.stroke, g.stroke) && fillEq(orig.fill, g.fill);
    else if (orig.kind === 'text' && g.kind === 'text')
      same = orig.text === g.text && vecEq(orig.at, g.at) && orig.angle === g.angle
...
export function writeLibSymbolNode(sym: LibSymbol): SList {
  ...
  const propNodes = (): SNode[] =>
    sym.properties.map((f) => (f.source.items.length > 0 ? patchProperty(f.source, f, true) : buildLibPropertyNode(f)));
  ...
  const unitNodes = (): SNode[] => (sym.extends !== undefined ? [] : sym.units.map(buildLibUnitNode));
  ...
  for (const it of src.items.slice(2)) {
    ...
    switch (head(it)) {
      case 'property':
        if (!propertiesWritten) { out.push(...propNodes()); propertiesWritten = true; }
        break;
      case 'symbol':
        if (!unitsWritten) { out.push(...unitNodes()); unitsWritten = true; }
        break;
      case 'power':
      ...
      case 'pin_numbers':
        if (sym.pinNumbersHidden)
          out.push(hasToken(it, 'hide') ? it : list(atom('pin_numbers'), list(atom('hide'), atom('yes'))));
        break;
      case 'pin_names': {
        const node = pinNamesNode(sym);
        if (node) out.push(node);
