...
import { iuToMM, mmToIU, type LibGraphic, type LibPin, type LibSymbol, type LibSymbolUnit, type SchField, type Vec2 } from '@ziroeda/core';
import { layoutText, measureText } from '../../../common/strokeFont.js';
import { ITALIC_TILT } from '@ziroeda/core';
...
export function pinBodyEnd(pin: Pick<LibPin, 'at' | 'angle' | 'length'>): Vec2 {
  switch (((pin.angle % 360) + 360) % 360) {
    case 0: return { x: pin.at.x + pin.length, y: pin.at.y };
    case 90: return { x: pin.at.x, y: pin.at.y - pin.length };
    case 180: return { x: pin.at.x - pin.length, y: pin.at.y };
    default: return { x: pin.at.x, y: pin.at.y + pin.length };
  }
}
...
export const PIN_SHAPE_NAMES: Record<string, string> = {
  line: 'Line', inverted: 'Inverted', clock: 'Clock', inverted_clock: 'Inverted clock',
  input_low: 'Input low', clock_low: 'Clock low', output_low: 'Output low',
  falling_edge_clock: 'Falling edge clock', non_logic: 'NonLogic',
};
...
function drawText(
  ...
  ctx.lineWidth = penWidth ?? (bold ? heightIU / 5 : Math.max(heightIU * 0.11, DEFAULT_LINE_WIDTH * 0.6));
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ctx.stroke(path);
  ctx.restore();
...
export function pinNameInfo(pin: LibPin, sym: { pinNamesHidden: boolean; pinNameOffset: number }): TextInfo | null {
  ...
  const info: TextInfo = { text: name, size, thickness, at: { x: 0, y: 0 }, vertical: false, halign: 'left', valign: 'center' };
...
export function pinNumberInfo(pin: LibPin, sym: { pinNumbersHidden: boolean; pinNamesHidden: boolean; pinNameOffset: number }): TextInfo | null {
  const number = pin.number === '~' ? '' : pin.number;
  if (number === '' || sym.pinNumbersHidden) return null;
  const size = pin.numberSize ?? DEFAULT_TEXT;
  if (size <= 0) return null;
  const thickness = clampTextPen(DEFAULT_LINE_WIDTH, size);
  const info: TextInfo = { text: number, size, thickness, at: { x: pin.length / 2, y: 0 }, vertical: false, halign: 'center', valign: 'bottom' };
...
export function pinElectricalTypeInfo(pin: LibPin): TextInfo {
  ...
  const info: TextInfo = {
    ...
    vertical: false, halign: 'right', valign: 'center',
  };
  transformTextForPin(info, pin);
  return info;
}
...
function drawTextInfo(ctx: CanvasRenderingContext2D, info: TextInfo, color: string): void {
  ...
  drawText(ctx, info.text, info.at, info.size, color, justify, info.vertical ? 90 : 0, false, false, info.thickness);
}
...
export function drawPin(
  ...
  if (pin.electricalType === 'no_connect') {
    ...
    line(ctx, p0, pos);
    line(ctx, { x: pos.x - TARGET_PIN_RADIUS, y: pos.y - TARGET_PIN_RADIUS }, { x: pos.x + TARGET_PIN_RADIUS, y: pos.y + TARGET_PIN_RADIUS });
    line(ctx, { x: pos.x + TARGET_PIN_RADIUS, y: pos.y - TARGET_PIN_RADIUS }, { x: pos.x - TARGET_PIN_RADIUS, y: pos.y + TARGET_PIN_RADIUS });
  } else {
    switch (pin.shape) {
      default:
      ...
      case 'falling_edge_clock':
        triLine(ctx,
          ...
          { x: p0.x - dir.x * clockSize, y: p0.y - dir.y * clockSize },
          { x: p0.x - dir.y * clockSize, y: p0.y + dir.x * clockSize });
        if (!dir.y) triLine(ctx, { x: p0.x + dir.x * diam, y: p0.y }, { x: p0.x + dir.x * diam, y: p0.y - diam }, p0);
        else triLine(ctx, { x: p0.x, y: p0.y + dir.y * diam }, { x: p0.x - diam, y: p0.y + dir.y * diam }, p0);
        line(ctx, p0, pos);
        break;
      case 'clock':
        line(ctx, p0, pos);
        if (!dir.y) triLine(ctx, { x: p0.x, y: p0.y + clockSize }, { x: p0.x - dir.x * clockSize, y: p0.y }, { x: p0.x, y: p0.y - clockSize });
        else triLine(ctx, { x: p0.x + clockSize, y: p0.y }, { x: p0.x, y: p0.y - dir.y * clockSize }, { x: p0.x - clockSize, y: p0.y });
        break;
      case 'input_low':
        line(ctx, p0, pos);
        if (!dir.y) triLine(ctx, { x: p0.x + dir.x * diam, y: p0.y }, { x: p0.x + dir.x * diam, y: p0.y - diam }, p0);
      ...
      case 'non_logic':
        line(ctx, p0, pos);
        line(ctx, { x: p0.x - (dir.x + dir.y) * radius, y: p0.y - (dir.y - dir.x) * radius },
          { x: p0.x + (dir.x + dir.y) * radius, y: p0.y + (dir.y - dir.x) * radius });
        line(ctx, { x: p0.x - (dir.x - dir.y) * radius, y: p0.y - (dir.x + dir.y) * radius },
  ...
  if (sym.showElectricalTypes) {
    drawTextInfo(ctx, pinElectricalTypeInfo(pin), hidden ? hiddenColor : SYMBOL_EDITOR_COLORS.privateNote);
  }
}

// ----- graphics ---------------------------------------------------------------

function drawArc3(ctx: CanvasRenderingContext2D, start: Vec2, mid: Vec2, end: Vec2, fill = false): void {
  const ax = start.x, ay = start.y, bx = mid.x, by = mid.y, cx = end.x, cy = end.y;
  const d = 2 * (ax * (by - cy) + bx * (cy - ay) + cx * (ay - by));
  if (Math.abs(d) < 1e-6) { line(ctx, start, end); return; }
...
const cssColor = (c: readonly [number, number, number, number]): string => `rgba(${c[0]}, ${c[1]}, ${c[2]}, ${c[3]})`;
...
export function drawGraphic(
  ...
  ctx.strokeStyle = shadow ? shadow.color
    : stroke?.color ? cssColor(stroke.color) : theme.symbolOutline;
  ctx.fillStyle = g.kind !== 'text' && g.fill?.type === 'color' && g.fill.color ? cssColor(g.fill.color)
    : g.kind !== 'text' && g.fill?.type === 'background' ? theme.symbolFill
    : theme.symbolOutline;
  ...
  switch (g.kind) {
    ...
    case 'polyline': {
      ...
      if (filled && !shadow) { ctx.closePath(); ctx.fill(); }
      ctx.stroke();
      break;
    }
    case 'text': {
      ...
      drawText(ctx, g.text, g.at, h, theme.symbolOutline, g.effects?.justify, (g.angle % 180) === 90 ? 90 : 0, g.effects?.bold, g.effects?.italic);
      break;
    }
  }
  ctx.setLineDash([]);
...
export function drawField(
  ...
  if (shadow) {
    ...
    const x0 = justify?.includes('right') ? f.at.x - w : justify?.includes('left') ? f.at.x : f.at.x - w / 2;
  ...
  drawText(ctx, shown, f.at, h, color, f.effects?.justify, (f.angle % 180) === 90 ? 90 : 0, f.effects?.bold, f.effects?.italic);
}
...
function drawGrid(ctx: CanvasRenderingContext2D, viewport: Viewport, theme: Theme, w: number, h: number): void {
...
export function renderSymbolScene(
  ...
  if (selection && selection.size > 0) {
    sym.units.forEach((u, ui) => {
      ...
      u.graphics.forEach((g, gi) => { if (selection.has(symItemId('gfx', ui, gi))) drawGraphic(ctx, g, theme, shadow); });
      u.pins.forEach((p, pi) => { if (selection.has(symItemId('pin', ui, pi))) drawPin(ctx, p, pinSettings, theme, shadow); });
    });
    sym.properties.forEach((f, fi) => {
      if (selection.has(symItemId('field', 0, fi))) drawField(ctx, f, theme, opts.showHiddenFields, shadow);
    });
  }
...
export function symbolBounds(sym: LibSymbol, unit: number, bodyStyle: number): { minX: number; minY: number; maxX: number; maxY: number } | null {
...
export function fitSymbol(sym: LibSymbol | null, unit: number, bodyStyle: number, canvasWidth: number, canvasHeight: number): Viewport {
