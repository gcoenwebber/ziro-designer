...
describe('PCB_SHAPE segment', () => {
  ...
  it('Move / Rotate / Flip', () => {
    ...
    const f = new PCB_SHAPE(SHAPE_T.SEGMENT, 'F.SilkS', {
      start: { x: 100, y: 0 },
      end: { x: 200, y: 0 },
      width: 50,
    });
...
describe('PCB_SHAPE rectangle', () => {
  it('unfilled: border live, interior not', () => {
    const s = new PCB_SHAPE(SHAPE_T.RECTANGLE, 'Edge.Cuts', {
      start: { x: 0, y: 0 },
      end: { x: 1000, y: 1000 },
      width: 40,
      filled: false,
  ...
  it('filled: interior hits', () => {
    const s = new PCB_SHAPE(SHAPE_T.RECTANGLE, 'F.Cu', {
      start: { x: 0, y: 0 },
      end: { x: 1000, y: 1000 },
      width: 0,
      filled: true,
...
describe('PCB_SHAPE circle', () => {
  it('filled interior vs ring', () => {
    const filled = new PCB_SHAPE(SHAPE_T.CIRCLE, 'F.Cu', {
      start: { x: 0, y: 0 },
      end: { x: 500, y: 0 },
      width: 20,
      filled: true,
    ...
    const ring = new PCB_SHAPE(SHAPE_T.CIRCLE, 'F.SilkS', {
      start: { x: 0, y: 0 },
      end: { x: 500, y: 0 },
      width: 20,
      filled: false,
...
describe('PCB_SHAPE arc', () => {
  ...
  const mk = (): PCB_SHAPE =>
    new PCB_SHAPE(SHAPE_T.ARC, 'F.SilkS', {
      start: { x: 1000, y: 0 },
      mid: { x: 707, y: 707 },
      end: { x: 0, y: 1000 },
      width: 100,
