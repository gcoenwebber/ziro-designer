/**
 * KiCad Newstroke stroke-font glyph data (ASCII + Latin-1, U+0020..U+00FF).
 *
 * Copied verbatim from KiCad common/newstroke_font.cpp (GPL-2.0-or-later). Each
 * string is a Hershey-encoded glyph: the first char pair gives the advance bounds
 * (char - 'R'), then coordinate pairs, with ' R' marking a pen-up between strokes.
 * Only the Latin range is included (the full font is ~3 MB of mostly-CJK glyphs);
 * these Latin glyphs are byte-identical to KiCad, so Latin text is pixel-faithful.
 */
