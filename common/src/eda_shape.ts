...
export enum SHAPE_T { UNDEFINED, SEGMENT, RECTANGLE, ARC, CIRCLE, POLY, BEZIER }
...
export class EDA_SHAPE {
  ...
  constructor(shape: SHAPE_T, opts: {
  ...
  move(aMoveVector: VECTOR2I): void {
    ...
    if (this.m_shape === SHAPE_T.POLY || this.m_shape === SHAPE_T.BEZIER) this.m_poly = this.m_poly.map(mv);
  }
  ...
  rotate(aRotCentre: VECTOR2I, aAngle: EDA_ANGLE): void {
    ...
    if (this.m_shape === SHAPE_T.POLY || this.m_shape === SHAPE_T.BEZIER) this.m_poly = this.m_poly.map(rp);
  }
  ...
  hitTest(aPosition: VECTOR2I, aAccuracy = 0): boolean {
    ...
    switch (this.m_shape) {
      ...
      case SHAPE_T.ARC: {
        ...
        const radius = this.GetRadius();
        if (this.IsFilledForHitTesting()) { if (dist > radius + maxdist) return false; }
      ...
      case SHAPE_T.RECTANGLE: {
        if (this.IsFilledForHitTesting()) {
          const x0 = Math.min(this.m_start.x, this.m_end.x) - maxdist, x1 = Math.max(this.m_start.x, this.m_end.x) + maxdist;
          const y0 = Math.min(this.m_start.y, this.m_end.y) - maxdist, y1 = Math.max(this.m_start.y, this.m_end.y) + maxdist;
          return aPosition.x >= x0 && aPosition.x <= x1 && aPosition.y >= y0 && aPosition.y <= y1;
        }
        const p = this.GetRectCorners();
        return TestSegmentHit(aPosition, p[0]!, p[1]!, maxdist) || TestSegmentHit(aPosition, p[1]!, p[2]!, maxdist)
            || TestSegmentHit(aPosition, p[2]!, p[3]!, maxdist) || TestSegmentHit(aPosition, p[3]!, p[0]!, maxdist);
      }
      case SHAPE_T.POLY:
      case SHAPE_T.BEZIER: {
        if (this.m_poly.length < 2) return false;
        if (this.IsFilledForHitTesting() && this.m_poly.length >= 3 && pointInPolygon(aPosition, this.m_poly)) return true;
        for (let i = 1; i < this.m_poly.length; i++) {
          if (TestSegmentHit(aPosition, this.m_poly[i - 1]!, this.m_poly[i]!, maxdist)) return true;
        }
        if (this.m_shape === SHAPE_T.POLY && this.m_poly.length >= 3
            && TestSegmentHit(aPosition, this.m_poly[this.m_poly.length - 1]!, this.m_poly[0]!, maxdist)) return true;
        return false;
      }
      default:
        return false;
  ...
  private arcSweepContains(relPos: VECTOR2I): boolean {
    ...
    const span = (from: number, to: number): number => { let d = to - from; while (d < 0) d += Math.PI * 2; return d; };
