...
import type { Schematic, LibSymbol, SchSymbol, Vec2 } from '../model/types.js';
 ...
 */
export function collectAnchors(sch: Schematic, libById: Map<string, LibSymbol>, exclude?: ReadonlySet<string>): Vec2[] {
  const pts: Vec2[] = [];
  sch.symbols.forEach((sym, i) => { if (!exclude?.has(refId('symbol', sym.uuid, i))) pts.push(...symbolPins(sym, libById.get(sym.libId))); });
  sch.lines.forEach((l, i) => { if (!exclude?.has(refId('line', l.uuid, i))) { pts.push(l.start); pts.push(l.end); } });
  sch.junctions.forEach((j, i) => { if (!exclude?.has(refId('junction', j.uuid, i))) pts.push(j.at); });
  sch.labels.forEach((l, i) => { if (!exclude?.has(refId('label', l.uuid, i))) pts.push(l.at); });
  return pts;
}

/** The connection points of the selected items only (used as move reference points). */
export function selectionAnchors(sch: Schematic, libById: Map<string, LibSymbol>, ids: ReadonlySet<string>): Vec2[] {
  const pts: Vec2[] = [];
  sch.symbols.forEach((sym, i) => { if (ids.has(refId('symbol', sym.uuid, i))) pts.push(...symbolPins(sym, libById.get(sym.libId))); });
  sch.lines.forEach((l, i) => { if (ids.has(refId('line', l.uuid, i))) { pts.push(l.start); pts.push(l.end); } });
  sch.junctions.forEach((j, i) => { if (ids.has(refId('junction', j.uuid, i))) pts.push(j.at); });
  sch.labels.forEach((l, i) => { if (ids.has(refId('label', l.uuid, i))) pts.push(l.at); });
  return pts;
}
