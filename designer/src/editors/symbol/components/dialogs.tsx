  ...
  return (
    <label className="row">
      ...
      <input
        ...
        onBlur={(e) => {
          const iu = parseMM(e.target.value);
          if (iu !== null) onChange(iu);
          setText(null);
        }}
        onKeyDown={(e) => {
          e.stopPropagation();
  ...
  const submit = (): void => {
    onOk({
      pin: {
        ...pin,
        name: name.trim(),
        number: number.trim().replace(/ /g, '_'),
        electricalType: etype,
        shape,
        angle,
        at: { x: posX, y: posY },
...
export function NewSymbolDialog({
  symbolNames,
  inheritFrom,
  onOk,
  onCancel,
  ...
  const submit = (): void => {
    const n = name.trim();
    if (n === '') {
      setError('Symbol must have a name.');
      return;
    }
    if (symbolNames.includes(n)) {
      setError(`Symbol '${n}' already exists in the library.`);
      return;
    }
    onOk({
      name: n,
      parentSymbolName: parent,
      reference: reference.trim() || 'U',
      unitCount: units,
      unitsInterchangeable: interchangeable,
      alternateBodyStyle: deMorgan,
      isPowerSymbol: power,
      excludeFromBom: exBom,
      excludeFromBoard: exBoard,
      pinNameInside: nameInside,
      pinTextPosition: pinTextPos,
      showPinNumber: showPinNum,
      showPinName,
    });
  };
...
export function LibSymbolPropertiesDialog({
  symbol,
  onOk,
  onCancel,
}: {
  ...
  const deleteRow = (): void => {
    ...
    if (!row) return;
    if (MANDATORY.has(row.key)) {
      setError('Mandatory fields cannot be deleted.');
      return;
    }
  ...
  const submit = (): void => {
    ...
    onOk({
      ...
      isPower: power,
      pinNameInside: nameInside,
      pinNameOffset: nameInside ? offset || mmToIU(0.0254) : 0,
      showPinNumbers: showNums,
      showPinNames: showNames,
    });
  };
...
export function ShapePropertiesDialog({
  initial,
  onOk,
  onCancel,
}: {
...
export function PinTableDialog({
  ...
  const submit = (): void => {
    ...
    for (const r of rows) {
      ...
      if (u?.pins[r.pinIdx]) u.pins[r.pinIdx] = r.pin;
    }
    next = { ...next, units };
    onOk(next);
  };
...
export function checkLibSymbol(sym: LibSymbol): string[] {
  ...
  pins.sort(
    (a, b) =>
      a.pin.number.localeCompare(b.pin.number) ||
      a.u.bodyStyle - b.u.bodyStyle ||
      a.u.unit - b.u.unit,
  );
  for (let i = 1; i < pins.length; i++) {
    ...
    msgs.push(
      `Duplicate pin ${n.pin.number}${pinName(n.pin.name)} at location ${mmXY(n.pin.at)} conflicts with pin ${p.pin.number}${pinName(p.pin.name)} at location ${mmXY(p.pin.at)}.`,
    );
  }
  ...
  for (const { pin } of pins) {
    if (!sym.isPower && pin.electricalType === 'power_in' && pin.hidden) {
      msgs.push(
        `Info: hidden power pin ${pin.number} at location ${mmXY(pin.at)}. (Hidden power pins will drive their pin names on to any connected nets.)`,
      );
    }
...
export function SymbolCheckDialog({
  symbol,
  onClose,
}: {
  symbol: LibSymbol;
