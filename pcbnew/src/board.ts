...
export interface BOARD_LAYER { id: number; name: string; type: string; userName?: string }
...
export class BOARD {
  ...
  Footprints(): FOOTPRINT[] { return this.m_footprints; }
  Tracks(): PCB_TRACK[] { return this.m_tracks; }
  Zones(): ZONE[] { return this.m_zones; }
  Drawings(): (PCB_SHAPE | PCB_TEXT)[] { return this.m_drawings; }

  GetLayers(): BOARD_LAYER[] { return this.m_layers; }
  SetLayers(layers: BOARD_LAYER[]): void { this.m_layers = layers; }

  GetNetInfo(): Map<number, string> { return this.m_nets; }
  SetNet(code: number, name: string): void { this.m_nets.set(code, name); }
  GetNetname(code: number): string { return this.m_nets.get(code) ?? ''; }

  /** BOARD::FlipLayer — the opposite-side layer (delegates to layer_ids). */
  FlipLayer(aLayer: PCB_LAYER_ID): PCB_LAYER_ID { return flipLayerImpl(aLayer); }
  ...
  Add(item: BOARD_TOP_ITEM): void {
    ...
    else if (item instanceof PCB_TRACK) this.m_tracks.push(item); // PCB_ARC / PCB_VIA are PCB_TRACKs
  ...
  Remove(item: BOARD_TOP_ITEM): void {
    const drop = <T>(arr: T[], it: T): void => { const i = arr.indexOf(it); if (i >= 0) arr.splice(i, 1); };
