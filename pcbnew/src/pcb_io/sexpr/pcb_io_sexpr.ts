...
const atNode = (p: VECTOR2I, angleDeg = 0): SList =>
  angleDeg ? list(atom('at'), atom(mm(p.x)), atom(mm(p.y)), atom(String(angleDeg)))
           : list(atom('at'), atom(mm(p.x)), atom(mm(p.y)));

const PAD_SHAPE_STR: Record<PAD_SHAPE, string> = {
  [PAD_SHAPE.CIRCLE]: 'circle', [PAD_SHAPE.RECTANGLE]: 'rect', [PAD_SHAPE.OVAL]: 'oval',
  [PAD_SHAPE.TRAPEZOID]: 'trapezoid', [PAD_SHAPE.ROUNDRECT]: 'roundrect',
  [PAD_SHAPE.CHAMFERED_RECT]: 'roundrect', [PAD_SHAPE.CUSTOM]: 'custom',
};
const PAD_ATTRIB_STR: Record<PAD_ATTRIB, string> = {
  [PAD_ATTRIB.PTH]: 'thru_hole', [PAD_ATTRIB.SMD]: 'smd', [PAD_ATTRIB.CONN]: 'connect', [PAD_ATTRIB.NPTH]: 'np_thru_hole',
};
...
function formatText(t: PCB_TEXT, tag: 'gr_text' | 'fp_text', localPos?: VECTOR2I, localAngle?: number): SList {
  const eda = t.m_eda;
  ...
  items.push(str(eda.GetText()), atNode(pos, ang), list(atom('layer'), str(t.GetLayer())), effectsNode(eda.GetTextSize(), 0));
  return { kind: 'list', items };
}

function formatShape(s: PCB_SHAPE, prefix: 'gr' | 'fp', xform?: (p: VECTOR2I) => VECTOR2I): SList {
  const kindName: Record<SHAPE_T, string> = {
    [SHAPE_T.SEGMENT]: 'line', [SHAPE_T.RECTANGLE]: 'rect', [SHAPE_T.CIRCLE]: 'circle',
    [SHAPE_T.ARC]: 'arc', [SHAPE_T.POLY]: 'poly', [SHAPE_T.BEZIER]: 'bezier', [SHAPE_T.UNDEFINED]: 'line',
  };
  const t = (p: VECTOR2I): VECTOR2I => (xform ? xform(p) : p);
  const items: SNode[] = [atom(`${prefix}_${kindName[s.GetShape()]}`)];
  if (s.GetShape() === SHAPE_T.CIRCLE) { items.push(xy('center', t(s.GetStart())), xy('end', t(s.GetEnd()))); }
  else { items.push(xy('start', t(s.GetStart())), xy('end', t(s.GetEnd()))); }
...
function formatPad(pad: PAD, fp: FOOTPRINT): SList {
  ...
  const local = RotatePoint({ x: pad.GetPosition().x - fp.GetPosition().x, y: pad.GetPosition().y - fp.GetPosition().y }, fpOrient.negate());
  const localAngle = pad.GetOrientation().sub(fpOrient).Normalize().AsDegrees();
  const items: SNode[] = [
    atom('pad'), str(pad.GetNumber()), atom(PAD_ATTRIB_STR[pad.GetAttribute()]), atom(PAD_SHAPE_STR[pad.GetShape()]),
...
function formatFootprint(fp: FOOTPRINT): SList {
  const items: SNode[] = [
    atom('footprint'), str(fp.GetFPID()),
    list(atom('layer'), str(fp.GetLayer())),
    atNode(fp.GetPosition(), fp.GetOrientation().AsDegrees()),
  ];
  const toLocal = (p: VECTOR2I): VECTOR2I => RotatePoint({ x: p.x - fp.GetPosition().x, y: p.y - fp.GetPosition().y }, fp.GetOrientation().negate());
  for (const f of fp.Fields()) {
    ...
    items.push(list(atom('property'), str(f.GetName()), str(f.GetText()), atNode(local, localAngle),
      list(atom('layer'), str(f.GetLayer())), effectsNode(f.m_eda.GetTextSize(), 0)));
  }
  ...
  for (const d of fp.GraphicalItems()) {
    ...
    else items.push(formatText(d, 'fp_text', toLocal(d.m_eda.GetTextPos()), d.m_eda.GetTextAngle().sub(fp.GetOrientation()).AsDegrees()));
  }
  return { kind: 'list', items };
}
...
function formatTrack(t: PCB_TRACK): SList {
  if (t instanceof PCB_VIA) {
    ...
    items.push(atNode(t.GetPosition()), list(atom('size'), atom(mm(t.GetWidth()))), list(atom('drill'), atom(mm(t.GetDrillValue()))),
      list(atom('layers'), str(t.GetLayer()), str(t.GetBottomLayer())), list(atom('net'), atom(String(t.GetNetCode()))));
    return { kind: 'list', items };
  }
  if (t instanceof PCB_ARC) {
    return list(atom('arc'), xy('start', t.GetStart()), xy('mid', t.GetMid()), xy('end', t.GetEnd()),
      list(atom('width'), atom(mm(t.GetWidth()))), list(atom('layer'), str(t.GetLayer())), list(atom('net'), atom(String(t.GetNetCode()))));
  }
  return list(atom('segment'), xy('start', t.GetStart()), xy('end', t.GetEnd()),
    list(atom('width'), atom(mm(t.GetWidth()))), list(atom('layer'), str(t.GetLayer())), list(atom('net'), atom(String(t.GetNetCode()))));
}

function formatZone(z: ZONE): SList {
  const items: SNode[] = [atom('zone'), list(atom('net'), atom(String(z.GetNetCode()))),
    { kind: 'list', items: [atom('layers'), ...z.GetLayerSet().map((l) => str(l))] }];
  if (z.GetOutline().length > 0) {
    items.push(list(atom('polygon'), { kind: 'list', items: [atom('pts'), ...z.GetOutline().map((p) => xy('xy', p))] }));
  }
  for (const [layer, rings] of z.GetFills()) {
    for (const ring of rings) {
      items.push(list(atom('filled_polygon'), list(atom('layer'), str(layer)),
        { kind: 'list', items: [atom('pts'), ...ring.map((p) => xy('xy', p))] }));
    }
  }
  return { kind: 'list', items };
}
...
export function formatBoardNode(board: BOARD): SList {
  ...
  items.push({ kind: 'list', items: [atom('layers'),
    ...board.GetLayers().map((l) => list(atom(String(l.id)), str(l.name), atom(l.type), ...(l.userName ? [str(l.userName)] : []))),
  ] });
  for (const [code, name] of board.GetNetInfo()) items.push(list(atom('net'), atom(String(code)), str(name)));
  ...
  for (const d of board.Drawings()) items.push(d instanceof PCB_SHAPE ? formatShape(d, 'gr') : formatText(d, 'gr_text'));
  return { kind: 'list', items };
}
